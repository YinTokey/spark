...
const script: Script = { id: 'script', title: 'Focus', text: 'Focus\n\nA complete spoken script.', ideaIds: ['idea'], status: 'Ready to record' };
const generated: GeneratedScript = { text: 'Focus\n\nA complete spoken script.', ideaIds: ['2b38059a-fb7f-4a21-a845-dc09b3045336'] };
...
function dependencies(command = false) {
  ...
  const consumeCaptureSlot = mock.fn(() => { events.push('rate'); return true; });
  ...
  return { repository, consumeCaptureSlot, transcribeAudio, parseScriptCommand, generateScript, events };
}
...
test('rate denial calls neither transcription nor generation', async () => {
  const deps = dependencies(true);
  deps.consumeCaptureSlot.mock.mockImplementation(() => false);
...
for (const [stage, code, status, failure] of [
  ...
  test(`${stage} failure maps to ${code} without retries or subsequent writes`, async () => {
    ...
    assert.ok(deps.consumeCaptureSlot.mock.callCount() <= 1);
