...
const ideaInput = z.object({ text: z.string().trim().min(1).max(8_000) }).strict();
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!demoRateLimiter.consume(session.userId, 'idea_write', 60)) throw new MutationError('rate_limited', 429, 'Too many saved ideas. Please wait an hour before trying again.');
    if (request.signal.aborted) throw new MutationError('request_cancelled', 409, 'Request was cancelled.');
    try {
      const idea = await repository.insertIdea(parsed.data.text, request.signal);
      status = 'saved';
      return privateJson({ idea });
    } catch {
      if (request.signal.aborted) throw new MutationError('request_cancelled', 409, 'Request was cancelled.');
      throw new MutationError('idea_save_failed', 502, 'Could not confirm that your idea was saved. Check your library before trying again.');
    }
  } catch (error) {
