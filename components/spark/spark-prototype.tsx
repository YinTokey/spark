          ...
          {recording ? <Teleprompter script={screen.script} onClose={() => setScreen({ kind: "script", script: screen.script })} /> : <div className="app-content" ref={content}>
            {screen.kind === "library" ? <>
              ...
              {screen.kind === "new" ? <><h1 className="detail-title" ref={heading} tabIndex={-1}>Catch a spark.</h1><IdeaForm full={ideas.length >= 50} onCancel={goHome} onSave={(title, note) => { setIdeas(current => [{ id: crypto.randomUUID(), title, note, date: "Today", time: "Just now", status: "Raw" }, ...current]); setTab("Ideas"); goHome(); }} /></> : screen.kind === "idea" ? <>
                ...
                <h1 className="detail-title" ref={heading} tabIndex={-1}>{screen.script.title}</h1><div className="script-status"><Badge status={screen.script.status} /><span><span aria-hidden="true">◷</span> {screen.script.duration} read</span></div><p className="updated-meta">{screen.script.ideaIds.length} ideas combined <span>·</span> Updated today</p>
                <section className="source-section"><h2>From these ideas</h2><div className="source-card">{screen.script.ideaIds.map(id => { const idea = ideas.find(item => item.id === id); return idea ? <div className="source-row" key={id}><Icon name="bulb" size={18} /><span>{idea.title}</span></div> : null; })}</div></section>
